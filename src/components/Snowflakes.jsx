   ...
   const particles = useMemo(
      () =>
         Array.from({ length: nbParticles }, (_, idx) => ({
            position: [randFloatSpread(11), randFloat(-18, 13), randFloatSpread(9)],
            rotation: [0, randFloat(0, Math.PI * 2), 0],
            size: randFloat(0.01, 0.09),
            lifetime: randFloat(1, 6),
            speed: randFloat(0.1, 1.2),
         })),
      []
   );
...
const colorStart = new Color("skyblue").multiplyScalar(55);
const colorEnd = new Color("white").multiplyScalar(55);
